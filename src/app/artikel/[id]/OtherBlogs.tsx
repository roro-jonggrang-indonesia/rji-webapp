      ...
      <Carousel className="mx-auto w-64 sm:w-full">
        <CarouselContent className="">
          {filterData.map((content: any, index: number) => (
            <OtherBlogsItem content={content} key={content.id} />
          ))}
        </CarouselContent>
        <CarouselPrevious />
        <CarouselNext />
...
function OtherBlogsItem({ content }: { content: any }) {
  return (
    <CarouselItem key={content.id} className="py-5 sm:basis-1/4">
      <Card className="rounded-xl border border-[#F5F7FA] bg-white shadow-lg">
        <CardHeader className="p-0">
          <Image
            src={`${process.env.NEXT_PUBLIC_BASE_URL}${content.attributes.cover_image.data.attributes.url}`}
            alt=""
            sizes="100vw"
            width={+`${content.attributes.cover_image.data.attributes.width}`}
            height={+`${content.attributes.cover_image.data.attributes.height}`}
